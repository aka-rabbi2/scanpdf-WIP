...
from headers_temp.utils.detect_func import detectTable, parameters
import argparse
#%%

def isHeader(data,headerName):
    for i in range(len(data)):
        if data.iloc[i,0]==headerName:
            data = data[i:]
            return data, False
    return data, True
...
def pdf_page2img(pdf_file, pg, save_image=True):
    img_page = convert_from_path(pdf_file, first_page=pg, last_page=pg)[0] #,poppler_path=r'C:\Users\1010820003\Desktop\poppler\poppler-0.68.0\bin'
    if save_image:
...
#%%
def detect_tables(opt, row_tol=10):
    cache = ()
    frozen = False
    ...
    opt=parameters(img_path, opt.weights)
    output_detect=detectTable(opt)   # 93 402 1619 1338 0 0.551024
    output=outpout_yolo(output_detect)

    os.remove(img_path)
    os.rmdir("headers_temp/outputs")
    ...
    output_camelot = camelot.read_pdf(
        filepath=pdf_file, pages=str(pg), flavor="stream", table_areas=interesting_areas, row_tol=row_tol
    )
    output_camelot=[x.df for x in output_camelot]
    for i,db in enumerate(output_camelot):
        # Clean df
        db.mask(db=='', inplace=True)
        #data = db.dropna(subset=[0],axis=1).dropna(subset=[0])
        db,frozen = isHeader(db,"SL.")
        cache += (db, frozen)
        # Clean df
